...
# ATTENTION -- INFO name '0' only for compatibility.
...
def analyze_results_2C_2_ranks(results_directory, output_directory, alpha=0.05):
    ...
    for result_file_id, result_file in enumerate(result_files):
        ...
        if global_results is None:
            ...
            extractor_names = results[Dims.EXTRACTORS.value].values
            n_extractors = len(extractor_names)
            outlier_detector_names = results[Dims.DETECTORS.value].values
            n_outlier_detectors = len(outlier_detector_names)
            spoiler_names = results[Dims.SPOILERS.value].values
            n_spoilers = len(spoiler_names)
            snrs = results[Dims.SNR.value].values
            n_snrs = len(snrs)
            spoiled_channels_fraction = results[
                Dims.CHANNEL_SPOIL_FRACTION.value
            ].values
            ...
            # n_files x metrics x extractors x detectors x spoilers x snr x ch_fraction x methods x folds
        ...
        for metric_id, metric_name in enumerate(metric_names):
            ...
            for extractor_id, extractor_name in enumerate(extractor_names):
                ...
                for outlier_detector_id, outlier_detector_name in enumerate(
                    ...
                    # files x metrics x extractors x detectors x spoilers x snr x ch_fraction x methods x folds
                    ...
                    c_mi = [f"{rename_classifiers(m[0])},{rename_classifiers(m[1])}({n})" for n, m in zip(range(1,n_methods+1), method_names)]
