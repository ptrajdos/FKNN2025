...
from dexterous_bioprosthesis_2021_raw_datasets_framework.estimators.meta.des.outiler_tools import (
    mask_outliers,
)
...
class DespOutlierFull(DespOutlier):
    ...
    def estimate_competence(self, query, neighbors, distances=None, predictions=None):
        base_competences = super().estimate_competence(
        ...
        for out_classifier_id in range(n_classifiers):
            outlier_mask[:, out_classifier_id] = self.pool_outlier_classifiers_[
                out_classifier_id
            ].predict(query)
