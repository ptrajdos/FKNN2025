...
from dexterous_bioprosthesis_2021_raw_datasets_framework.estimators.meta.des.des_p_outlier import (
    DespOutlier,
)
from dexterous_bioprosthesis_2021_raw_datasets_framework.estimators.meta.outlier.outlier_detector_combiner_min import (
    OutlierDetectorCombinerMin,
)
...
class ChannelCombinationClassifierOutliersFast2(BaseEstimator, ClassifierMixin):
    ...
    def __init__(
        ...
        es_class -- Class of the meta classifier that is able to use an ensemble of one class classifiers.
    ...
    def _prepare_no_fault_model(self, X, y):
        """
        Prepares model to be used when no outlier channels detected.
        The model is a single classifier that uses all features.
        """
        ...
        self.no_fault_detector_ = OutlierDetectorCombinerMin(
            self.base_outlier_detectors_,
        )

    def _create_ensemble(self, X, y):
        """
        Prepares the multi-class and one-class ensembles
        ...
        n_channels = len(self.channel_features)
        channel_groups = self.channel_combination_generator(
            range(n_channels), **self._channel_combination_generator_options
        )
        ...
        for channel_group_id, channel_group_indices in enumerate(channel_groups):
            # create base multi-class classifiers
            selected_channels_features = self._select_channel_group_features(
                channel_group_indices
            )
            self.base_classifiers_.append(
                Pipeline(
                    [
                        (
                            "trans",
                            SelectAttributesTransformer(
                                column_indices=selected_channels_features
                            ),
                        ),
            ...
            outlier_detectors = self.base_outlier_detectors_[
                list(channel_group_indices)
            ]
        ...
        self.ensemble_ = self.es_class(
            pool_classifiers=self.base_classifiers_,
    ...
    def predict(self, X, y=None):
        ...
        if self.use_no_fault_model:
            ...
            outlier_object_list = [
                idx for idx, k in enumerate(outlier_indicator) if k < 0
            ]
            non_outlier_object_list = [
                idx for idx, k in enumerate(outlier_indicator) if k >= 0
            ]
            n_objects = len(X)
            ...
            if len(outlier_object_list) > 0:
                out[outlier_object_list] = self.label_encoder_.inverse_transform(
                    self.ensemble_.predict(X_outlier)
                )
            if len(non_outlier_object_list) > 0:
                out[non_outlier_object_list] = self.label_encoder_.inverse_transform(
                    self.no_fault_model_.predict(X_non_outlier)
                )
