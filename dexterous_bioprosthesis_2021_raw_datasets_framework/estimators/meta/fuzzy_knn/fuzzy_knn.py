...
class FuzzyKNN(BaseEstimator, ClassifierMixin):
    ...
    def _create_similarity_matrix(self, X):
        ...
        if self.n_neighbors is not None:
            for channel in range(n_channels):
                channel_matrix = sim_matrix[:, :, channel]
                top_indices = np.argsort(channel_matrix, axis=1)[:, -self.n_neighbors :]
                mask = np.zeros_like(channel_matrix, dtype=bool)
                np.put_along_axis(mask, top_indices, True, axis=1)
                sim_matrix[:, :, channel] *= mask
    ...
    def _combine_channel_predictions(self, inference_matrix):
        ...
        """
        combined_predictions = np.mean(inference_matrix, axis=2)
    ...
    def predict_proba(self, X):
        ...
        inference_matrix = self.t_norm_.t_norm(
